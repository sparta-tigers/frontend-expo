...
test("Property 16: Checklist Evaluation", () => {
  const file: SourceFile = {
    absolutePath: "/p.ts", relativePath: "p.ts", extension: ".ts",
    content: "export const A = 1;\n".repeat(10)
  };

  const genFile: SourceFile = {
    absolutePath: "/p/index.ts", relativePath: "p/index.ts", extension: ".ts",
    content: "export const A = 1;"
  };
  ...
  const input: VerificationInput = {
    ...
    generatedModules: [
      {
        plan: { path: "p/index.ts", concern: "Logic", expectedLoc: 1, publicApi: ["A"] },
        file: genFile
      }
    ],
    barrelFile: {
      plan: { path: "p/barrel.ts", reExports: ["A"] },
      file: { absolutePath: "/p/barrel.ts", relativePath: "p/barrel.ts", extension: ".ts", content: "export { A } from './index';" }
    },
