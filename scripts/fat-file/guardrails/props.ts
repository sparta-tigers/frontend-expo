  ...
  for (const file of generatedFiles) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/\b[a-zA-Z0-9_]+=\{\{/.test(line)) {
        violations.push(`File ${file.relativePath}:${i + 1} - Inline object prop detected. Prefer useMemo or stable reference.`);
      } else if (/\b[a-zA-Z0-9_]+=\{\[/.test(line)) {
        violations.push(`File ${file.relativePath}:${i + 1} - Inline array prop detected. Prefer useMemo or stable reference.`);
      } else if (/\b[a-zA-Z0-9_]+=\{(?:\([^)]*\)|[^=]*)\s*=>/.test(line) || /\b[a-zA-Z0-9_]+=\{function\b/.test(line)) {
        violations.push(`File ${file.relativePath}:${i + 1} - Inline function prop detected. Prefer useCallback or stable reference.`);
      }
    }
  }
