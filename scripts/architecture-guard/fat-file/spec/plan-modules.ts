...
export function planModules(diagnosis: Diagnosis): ModulesPlanResult {
  ...
  const expectedLocPerConcern = Math.ceil(
    diagnosis.loc / Math.max(1, diagnosis.concerns.length),
  );
  ...
  const decomposition: ModulePlan[] = diagnosis.concerns.map((concern) => {
    ...
    const isIndex =
      diagnosis.relativePath.endsWith("/index.tsx") ||
      diagnosis.relativePath.endsWith("/index.ts");
  ...
  const exception: RefactoringException | undefined = needsException
    ? {
        reason: `예상 LoC가 300줄을 초과합니다 (${expectedLocPerConcern}줄). 추가적인 분할 설계가 필요합니다.`,
  ...
  const isIndex =
    diagnosis.relativePath.endsWith("/index.tsx") ||
    diagnosis.relativePath.endsWith("/index.ts");
  const barrelPath = isIndex
    ? diagnosis.relativePath
    : `${diagnosis.relativePath.split("/").slice(0, -1).join("/")}/${featureId}/index.ts`;
