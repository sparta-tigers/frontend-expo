...
export const useScheduleScreen = () => {
  ...
  const year = useMemo(() => params.year ? parseInt(params.year) : getCurrentYear(), [params.year]);
  const month = useMemo(() => params.month ? parseInt(params.month) : getCurrentMonth(), [params.month]);
